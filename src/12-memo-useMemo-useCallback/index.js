...
const mostExpensive = useMemo(()=> calculateMostExpensive(products),[products])
    ...
    return (
        <>
            ...
            <h1 style={{marginTop: '20px'}}>Cart: {cart}</h1>
...
// no re-render! beceause memo memorize it was render once!!!
...
const SingleProduct = ({fields, addToCart}) => {
    ...
    return (
        <article className="product">
            ...
            <h4>{name}</h4>
            <p>${price}</p>
            <button className="btn" onClick={addToCart}>Add item to cart</button>
        </article>
    )
}
